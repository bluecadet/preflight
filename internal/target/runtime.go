...
type remoteModule interface {
	Check(ctx context.Context, params map[string]any) (needed bool, message string, err error)
...
func (m remoteModuleFuncs) Check(ctx context.Context, params map[string]any) (bool, string, error) {
...
func executeRemoteModule(
	...
	needsChange, checkMessage, err := mod.Check(ctx, params)
	if err != nil {
		return Result{TaskID: taskID, Status: StatusFailed, Error: err}, err
	}
	if !needsChange {
		...
		return Result{TaskID: taskID, Status: StatusOK, Message: message}, nil
	}
	if dryRun {
	...
	if trimmed := strings.TrimSpace(applyOutput); trimmed != "" {
		...
		result.Output = splitOutputLines(applyOutput)
		if onOutput != nil {
