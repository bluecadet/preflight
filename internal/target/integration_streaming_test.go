...
// TestIntegration_Streaming exercises the output streaming path over every
// configured transport (WinRM and/or SSH-to-Windows). Each transport gets its
// own subtest and is independently skipped when its env vars are unset.
//
// It runs a multi-line PowerShell command through the powershell module,
// asserts that every line is delivered through the onOutput callback in order,
// and — when the transport supports it — that the lines arrive incrementally
// rather than in a single batch at completion.
//
// Incremental delivery is a capability of the underlying session, not of
// preflight: WS-Man (WinRM) receive channel buffers stdout in the WinRM
// service's internal pipe buffer and delivers in batches when the command
// finishes, while SSH forwards output as it is produced. The test script
// sleeps 100ms between each of five chunks, so genuinely streamed output
// spreads over ~400ms while batched output collapses to ~0. The test stamps
// each callback; if the spread shows the session batched, it skips the
...
func TestIntegration_Streaming(t *testing.T) {
	forEachTransport(t, func(t *testing.T, _ PowerShellRunner, tgt Target) {
...
Write-Output 'done'`
		...
		// Run Execute in a goroutine, stamping the arrival time of each
		// onOutput callback so we can measure how the output is spread
		// over time.
		...
		// session delivered everything in one batch at completion — an environment
		// limitation, not a preflight defect — so skip rather than fail.
		const minStreamingSpread = 250 * time.Millisecond
		spread := got[len(want)-1].at.Sub(got[0].at)
		if spread < minStreamingSpread {
			t.Skipf("session delivered output in a single batch (spread %v across %d lines); "+
				"incremental streaming is not supported on this connection. "+
				"Delivery and ordering were verified.", spread, len(want))
		}
	})
}