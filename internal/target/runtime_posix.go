...
type posixShellBackend interface {
	...
	// InitSystem returns the cached POSIX init-system signal from the runtime
	// detection probe ("systemd" or "" when absent / not probed). Modules that
	// depend on systemd read this rather than re-probing, preserving the single
	// detection path (spec §6).
	InitSystem() string
}
...
func newPOSIXShellRegistry(backend posixShellBackend) ModuleRegistry {
	supported := ModuleRegistry{
		...
		"service": moduleFuncs{
			check: func(ctx context.Context, params map[string]any, _ OutputFunc) (CheckResult, error) {
				return checkPOSIXService(ctx, backend, params)
			},
			apply: func(ctx context.Context, params map[string]any, _ OutputFunc) (ApplyResult, error) {
				return ApplyResult{}, applyPOSIXService(ctx, backend, params)
			},
		},
	}
