...
Re-run the install script to replace the installed binary with the latest
release:
...
On Windows, delete the install directory and, if you want, remove it from
...
Projects you created (playbooks, `preflight.yml`, `state/`, `secrets/`)
...
Verification is optional for everyday installs — the install scripts
already check the archive digest against the published checksum file. Use
this section when your environment requires manually verified artifacts.
